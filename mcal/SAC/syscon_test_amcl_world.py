...
class StageWorld():
    ...
    def laser_scan_callback(self, scan):
        ...
        #scan_foward_range = self.scan_origin[200:310]
        #scan_inflation = self.scan_origin

        #print("data {}, {}, {}, {}, {}".format(self.scan[0], self.scan[76], self.scan[256], self.scan[436], self.scan[511]))


        '''
        scan_min_data = np.min(scan_foward_range)
        scan_min_index = np.where(scan_foward_range == scan_min_data)
        
        if not scan_min_data == 10.0:
            for i in range(5):
                inflation_index = scan_min_index[0][0] - 2 + i
                if(inflation_index > -1 and inflation_index < 110):
                    scan_inflation[140+inflation_index] = scan_min_data - 0.2
        
        
        '''
        '''

        scan_min_data = np.min(scan_inflation)
        scan_min_index = np.where(scan_inflation == scan_min_data)
        if not scan_min_data < 5.0:
            for n_index in range(len(scan_min_index)):
                for i in range(9):
                    inflation_index = scan_min_index[n_index][0] - 4 + i
                    if(inflation_index > -1 and inflation_index < 512):
                        scan_inflation[inflation_index] = scan_min_data - 0.4
        '''
