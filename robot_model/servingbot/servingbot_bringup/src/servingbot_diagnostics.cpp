...
#include <servingbot_msgs/SensorState.h>
#include <servingbot_msgs/VersionInfo.h>
#include <string>
...
void sensorStateMsgCallback(const servingbot_msgs::SensorState::ConstPtr &msg)
{
  setBatteryDiagnosis(diagnostic_msgs::DiagnosticStatus::WARN, "Charge!!! Charge!!!");

  if (msg->button == servingbot_msgs::SensorState::BUTTON0)
    setButtonDiagnosis(diagnostic_msgs::DiagnosticStatus::OK, "BUTTON 0 IS PUSHED");
  else if (msg->button == servingbot_msgs::SensorState::BUTTON1)
...
void firmwareVersionMsgCallback(const servingbot_msgs::VersionInfo::ConstPtr &msg)
{
  ...
  if (check_version == false)
  {
    if (firmware_version.major_number == FIRMWARE_VERSION_MAJOR_NUMBER)
    {
      if (firmware_version.minor_number > FIRMWARE_VERSION_MINOR_NUMBER)
      {
        ...
        ROS_WARN("You can find how to update its in `FAQ` section(servingbot.robotis.com)");
      }
    }
    else
    {
  ...
  servingbot_msgs::VersionInfo version;
...
{
  ros::init(argc, argv, "servingbot_diagnostic");
  ros::NodeHandle nh;

  tb3_diagnostics_pub  = nh.advertise<diagnostic_msgs::DiagnosticArray>("diagnostics", 10);
  tb3_version_info_pub = nh.advertise<servingbot_msgs::VersionInfo>("version_info", 10);
