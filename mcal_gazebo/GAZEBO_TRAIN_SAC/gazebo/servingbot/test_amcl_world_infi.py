...
class StageWorld():
    def __init__(self, beam_num, index, num_env, robot_radius):
        ...
        self.goal_two_point = [[0, 0], [0, 0]]
        self.goal_point_count = 0
        self.goal_index = 0
    ...
    def goal_callback(self, Goal):
        ...
        if self.goal_point_count == 0:

            print("first goal")

            self.goal_two_point[0][0] = Goal.pose.position.x
            self.goal_two_point[0][1] = Goal.pose.position.y
            print(self.goal_two_point[0][0])
            self.goal_point_count = 1

        elif self.goal_point_count == 1:

            print("second goal")

            self.goal_two_point[1][0] = Goal.pose.position.x
            self.goal_two_point[1][1] = Goal.pose.position.y
            
            self.goal_point_count = 2

            self.goal_index = 1

            self.goal_point[0] = self.goal_two_point[0][0]
            self.goal_point[1] = self.goal_two_point[0][1]
            self.pre_distance = 0
    ...
    def laser_scan_callback(self, scan):
        ...
        map_scan = np.zeros(512)
        
        j = 0

        for i in range(767):
            if not i % 3 == 0:
                map_scan[j] = full_scan[i+22]
                j = j + 1
        
        self.scan =  copy.deepcopy(map_scan)
        '''
