...
from syscon_multi_agents_world import StageWorld
...
def run(comm, env, agent, policy_path, args):
    ...
    for i_episode in range(args.num_steps):
        ...
        while not done and not rospy.is_shutdown():    
            ...
            if env.index == 0 and args.eval == False:
        ...
        print("rank: {}, Episode: {}, episode steps: {}, reward: {}, result: {}".format(env.index, i_episode, episode_steps, round(reward, 2), result))
...
if __name__ == '__main__':
    ...
    if rank == 0:
        ...
        action_bound = spaces.Box(low=np.array([-0.1, -1.0]), high=np.array([0.80, 1.00]), dtype=np.float32)
        ...
        file_policy = policy_path + '/policy_epi_8100'
        file_critic_1 = policy_path + '/critic_1_epi_8100'
        file_critic_2 = policy_path + '/critic_2_epi_8100'
